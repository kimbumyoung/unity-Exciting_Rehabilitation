...
public class Create : MonoBehaviour {
	...
	void Start(){
		...
		Debug.Log (Db.scoreArray[0]);
		Debug.Log (Db.scoreArray[1]);
		Debug.Log (Db.scoreArray[2]);
		max = Db.scoreArray.Length;
		Debug.Log(max);
		//this.gameObject.GetComponent<Db> ().SelectUsingReader ();
	...
	{		int i=6;
		while (true) {
			...
			pos = new Vector3 (x, (Convert.ToInt32(Db.scoreArray[i])*0.005f), 0);
          ...
          //  GameObject.Find("t" + Number1.ToString()).GetComponent<Text>().text = "999";
			cc = Number.ToString();
            GameObject.Find(cc).GetComponent<Text>().text = Db.scoreArray[i];
