...
public class Managerment : MonoBehaviour {
	...
	void Update () {
		...
		if (GameObject.Find ("TimeLabel").GetComponent<TimeControl> ().s.value == 0) {
			Debug.Log("sdsd");
			sendScore = score.ToString();
			sendTable = "Insert into armgame(Score) values ('";
			GameObject.Find("Main Camera").GetComponent<Db>().score1 = sendScore;
			GameObject.Find("Main Camera").GetComponent<Db>().ArmTable = sendTable;
			GameObject.Find("Main Camera").GetComponent<Db>().SendData();
			score = 0;
			Destroy(GameObject.Find("share2"));
			Application.LoadLevel(0);
